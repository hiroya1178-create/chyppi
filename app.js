...
function inferProjectKeywords(projectName){
  ...
  return {
    ...
    hasLeaflet: text.includes("チラシ") || text.includes("フライヤー"),
  };
}
function getAssignmentSuggestion(orderLike){
  ...
  const candidates = state.staff.map(member => {
    ...
    if(keys.hasLeaflet && (skills.includes("バナー") || skills.includes("ui"))) skillScore += 2;

    if(String(member.role || "").includes("ディレクター")) skillScore += 1;
    if(String(member.role || "").includes("デザイナー") && (keys.hasBanner || keys.hasLp || keys.hasWeb || keys.hasSns || keys.hasLeaflet)) skillScore += 1;
  ...
  return { name: best.member.name, load: best.load, reason: reasons.join(" / ") };
}
...
function SideNav({ currentPage, setCurrentPage }){
  ...
  return `<aside class="sidebar"><div class="brand">デザインマネージャー</div><div class="brand-sub">クリエイティブ管理</div><div class="nav">${items.map(([k,l])=>`<button class="${currentPage===k?'active':''}" onclick="setPage('${k}')">${l}</button>`).join("")}</div><div class="version">デザインマネージャー 公開版 v3.6.0</div></aside>`;
}
...
const state = {
  ...
  createForm: { client:"", projectName:"", amount:"80000", estimatedHours:"12", assignee:staffOptions[0], finishDate:"2026-03-31", status:"納期OK", notes:"", sourceFileName:"" },
...
function openCreate(prefillTemplateId=null){
  ...
  if(prefillTemplateId){
    ...
    state.createForm = { client:"", projectName:tpl?.title || "", amount:String(tpl?.price || 80000), estimatedHours:String(tpl?.hours || 12), assignee:staffOptions[0], finishDate:"2026-03-31", status:"納期OK", notes:"", sourceFileName:"" };
  } else {
    state.createForm = { client:"", projectName:"", amount:"80000", estimatedHours:"12", assignee:staffOptions[0], finishDate:"2026-03-31", status:"納期OK", notes:"", sourceFileName:"" };
  }
  render();
}
